      ...
      <div id="content">
        ...
        <?php 
          ...
          foreach ($ngayarr as $key => $value) {
            ...
            // SUM(CASE WHEN date(lh.`ProPlanDate`) = '2020-02-17' THEN lh.`ProPlanQuantity` ELSE 0 END) AS TotalOk1
            $sumtext = $sumtext."
            SUM(CASE WHEN date(lh.`ProPlanDate`) = '".date('Y-m-d',strtotime($value['end']))."' THEN lh.`ProPlanQuantity` ELSE 0 END) AS TotalOk".$date.",
            ";
          }
