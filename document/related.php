  ...
  <?php require('sidebar.php') ?>
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <!-- Main Content -->
        ...
        <div class="container">
        <?php
        $thisdoc = $sDB -> query("Select * from Document where DocumentId=?",$_GET['id'])->fetchArray();
        $lastdoc = $sDB -> query("Select * from DocumentDetail where DocumentId=?",$_GET['id'])->fetchArray();
        // var_dump($lastdoc);
        ?>
        <h1>Tiêu chuẩn : <?php echo $thisdoc['DocumentName'] ?></h1>

        <table class="table">
            <tr>
              <th>Tên tiêu chuẩn</th>
              <td><?php echo $thisdoc['DocumentName'] ?></td>
            </tr>
            <tr>
              <th>Mã</th>
              <td><?php echo $thisdoc['DocumentNumber'] ?></td>
            </tr>
            <tr>
            <th>Miêu tả</th>
              <td><?php echo $thisdoc['DocumentDescription'] ?></td>
            </tr>
            <th>Phiên bản</th>
              <td><?php echo $lastdoc['DocumentDetailVersion'] ?></td>
            </tr>

            <th style='width: 20%'>Tải liệu</th>
            <?php
             $filename = $lastdoc['DocumentDetailFileName'];
             $tmp = explode(".", $filename);
             $ext = end($tmp);
             echo "<td ><a target='_blank' href='files/".$lastdoc['DocumentDetailId'].'.'.$ext."'>".$lastdoc['DocumentDetailFileName']."</a></td";
            ?>
              <!--  -->
            </tr>
        </table>

        <h3>Các tiêu chuẩn liên quan</h3>
          <div class="col-md-12">
            ...
            <tbody>
                  <?php
                  $sql = "Select ad.*,s.*,d.* from DocumentAD ad
                  inner join Document d on d.DocumentId = ad.RelatedDocumentId 
                  inner join section s on s.SectionId= d.SectionId
                  where ad.DocumentId=?";
                  $list2 = $sDB->query($sql,$_GET['id'])->fetchAll();
