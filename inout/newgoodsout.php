...
$page_css='th,td{font-weight: normal;font-size: 13px;text-align: center;vertical-align: middle !important;}.vs__dropdown-toggle {border: 0px !important;margin-top: -4px;} .vs__selected{white-space: nowrap;max-width: 200px;overflow: hidden;font-size: 14px;} .vs__dropdown-menu li{font-size: 14px;}input::placeholder{font-size: 14px;} .vmoney{width: 100px; font-size: 16px;}';
    ...
    <div id="content-wrapper" class="d-flex flex-column">
      ...
      <div id="content">
        ...
        <div class="mx-1">
          ...
          <form action="listen-goodsout.php" method="post">
            <input type="hidden" v-model="GoodsOutputsId" name="GoodsOutputsId">
            <div class="form-group row">
              ...
              <div class="col-sm-3">
                <select class="form-control" name="GoodsOutputsType" v-model="GoodsOutputsType">
              ...
              <div class="col-sm-3">
                <input readonly type="text" class="form-control" v-model="GoodsOutputsNo" name="GoodsOutputsNo">
              </div>
              ...
              <div class="col-sm-2">
                <input type="date" class="form-control" v-model="GoodsOutputsDate" name="GoodsOutputsDate" required>
              </div>
            ...
            <div class="w-100" style="overflow-y:auto;overflow-x:none;">
              <table style="margin-bottom: 70px;" class="table table-bordered">
                ...
                <tbody>
                  <tr v-for="(item,index) in GoodsOutputitems">
                    <td>{{index+1}}</td>
                    <td>
                      <v-select 
                    ...
                    <td><input style="height: 33px; font-size: 16px; width: 130px;" type="text" v-model="item.GoodsOutputItemsWo" name="GoodsOutputItemsWo[]"></td>
                    <td><money v-model="item.GoodsOutputItemsUnitPrice" v-bind="money" class="vmoney" required></money>
                    <input type="hidden" name="GoodsOutputItemsUnitPrice[]" :value="item.GoodsOutputItemsUnitPrice">
                    <td>{{productSelected(item).ProductsUnit}}</td>
                    <td><input style="height: 33px; font-size: 16px;width: 60px;" type="number" v-model="item.GoodsOutputItemsQty" name="GoodsOutputItemsQty[]"></td>
                    <td>{{(item.GoodsOutputItemsQty * item.GoodsOutputItemsUnitPrice).format()}}</td>
                    <td><input style="height:33px; font-size: 16px;" type="text" v-model="item.GoodsOutputItemsRemark" name="GoodsOutputItemsRemark[]"></td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr>
            ...
            <div class="row pb-5 pt-3">
              <div class="col-md-6 d-flex">
                ...
                <a :href="'print-stockout.php?id='+GoodsOutputsId" target="_blank" class="btn btn-secondary mt-auto"><i class="fas fa-print"></i></a>
              </div>
            </form>
          </div>
        </div>
    ...
    $(function () {
      ...
      new Vue({
        ...
        methods: {
          ...
          removeLastItem(){
            if(this.GoodsOutputitems.length == 0)
            {
              return;
            }
            this.GoodsOutputitems.splice(-1,1);
          },
          removeItem(index){
            if(this.GoodsOutputitems.length == 0)
            {
            ...
            this.GoodsOutputitems.splice(index,1);
          },
          loadPlans(){
        ...
        created: function(){
          axios.get('/smes/inout/loadgoodsoutdata.php?mtpid='+this.MaterialTypesId).then(({data}) => {
            this.products_data = data['products_data'];
            this.GoodsOutputsId = data['GoodsOutputsId'];
            this.FromId = data['FromId']==0?'':data['FromId'];
            this.ToId = data['ToId']==0?'':data['ToId'];
            this.GoodsOutputsType = data['GoodsOutputsType']+'';
            this.GoodsOutputsDate = data['GoodsOutputsDate'];
            this.GoodsOutputsNo = data['GoodsOutputsNo'];
            this.GoodsOutputitems = data['GoodsOutputitems'];
          }).catch(() => {
            console.log('error');
          });
        }
